...
- `packages/codex-plugin-proto`: pure Codex shell bootstrap protocol layered on `distribution-proto`. It owns stable bootstrap argv/env names, acquisition/fixture manifest parsing, Codex shell state paths, handoff descriptors, ready messages, and transition validation without owning shared suite paths, runtime leases, filesystem I/O, downloads, process spawning, MCP transport, or daemon calls.
...
- `packages/distribution-proto`: shell-neutral Open Design suite substrate shared by packaged/desktop, launcher, tools-pack, and Codex plugin delivery. It owns channel/namespace coordinates, canonical suite/runtime-store paths, runtime pointers/bindings/leases, runtime/shell identity, build artifact reports, digest/version validation, portable inventory paths, path containment, and exact identity comparison without owning an updater, installer, MCP transport, filesystem mutation, or process lifecycle.
