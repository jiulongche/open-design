...
- `packages/codex-plugin-proto`: pure Codex shell protocol layered on `distribution-proto`. It owns stable launcher args/runtime env names, supported platform targets, the isolated `codex-plugin/{runtime,updates,cache,logs,state}` layout, acquisition/control manifest parsing, handoff descriptors, ready messages, and transition validation without owning filesystem I/O, downloads, process spawning, MCP transport, or daemon calls.
