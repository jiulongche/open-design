...
- `packages/codex-plugin-proto`: pure Codex shell bootstrap protocol layered on `distribution-proto`. It owns stable bootstrap argv/env names, the isolated `codex-plugin/{environment,runtime,updates,cache,logs,state}` layout, acquisition/control and environment manifest parsing, handoff descriptors, ready messages, and transition validation without owning filesystem I/O, downloads, process spawning, MCP transport, or daemon calls.
...
- `packages/distribution-proto`: shell-neutral Open Design suite substrate shared by packaged/desktop, launcher, tools-pack, and Codex plugin delivery. It owns channel/namespace/data coordinates, reusable runtime-store paths and pointer/binding/lease shapes, runtime/shell identity, build artifact reports, digest/version validation, portable inventory paths, path containment, and exact identity comparison. Consumers decide whether lifecycle state is shared or isolated; the package does not own an updater, installer, MCP transport, filesystem mutation, or process lifecycle.
...
pnpm --filter @open-design/codex-plugin-proto typecheck
pnpm --filter @open-design/codex-plugin-proto test
...
pnpm --filter @open-design/distribution-proto typecheck
