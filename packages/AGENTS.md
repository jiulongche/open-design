...
- `packages/distribution-proto`: shell-neutral Open Design suite substrate shared by packaged/desktop, launcher, tools-pack, and Codex plugin delivery. It owns channel/namespace coordinates, the canonical namespace/data/cache/log/runtime/update path resolver, runtime/shell identity, digest/version validation, portable inventory paths, path containment, and exact identity comparison without owning an updater, installer, MCP transport, filesystem mutation, or process lifecycle.
