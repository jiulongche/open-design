...
unit coverage, broad E2E Vitest, and the complete five-shard UI P0 matrix. It
...
remains the full five-shard matrix in PR and merge-queue plans; no job reads
...
Its candidate keeps `entry-settings`, both `project-workspace-*` shards,
...
out-of-surface change falls back to the full five-shard matrix and records the
