...
- `apps/codex-plugin`: Codex host shell source. It owns the Codex plugin manifest/skills, static stdio MCP bootstrap, and lazy runtime launcher/handoff, while consuming the same Open Design channel/namespace/path/runtime-store substrate as packaged/desktop. It does not redefine suite storage through Codex host paths, bind runtime state to `CODEX_HOME`, or consume Desktop-only lifecycle state.
