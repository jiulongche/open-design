...
- `apps/codex-plugin`: Codex host shell source. It owns the Codex plugin manifest/skills, Node-independent platform bootstrap, static stdio MCP entry, and lazy runtime launcher/handoff. It consumes the same Open Design channel/namespace/data coordinates as packaged/desktop, but owns independent environment/runtime/update/cache/log/state roots below `namespaces/<namespace>/codex-plugin/`. It does not bind product state to `CODEX_HOME` or consume Desktop-only lifecycle state.
