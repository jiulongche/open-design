...
- `apps/codex-plugin`: Codex host shell source. It owns the Codex plugin manifest/skills, platform-targeted plugin-native execution carrier, static stdio MCP entry, and lazy runtime launcher/handoff. Carrier, MCP bundle, skills, and identity update only through native plugin installation or replacement. It consumes the same Open Design channel/namespace/data coordinates as packaged/desktop, but owns independent runtime/update/cache/log/state roots below `namespaces/<namespace>/codex-plugin/`. It does not bind product state to `CODEX_HOME` or consume Desktop-only lifecycle state.
