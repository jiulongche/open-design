...
`,
  ...
  it("[P2] routes trusted Linux CI through the Nexu runner fleet", async () => {
    ...
    expect(uiP0).toContain("matrix.shard == 'project-workspace-collab'");
    expect(uiP0).toContain("Preserve project-workspace-collab domain artifact");
    expect(workflow).not.toContain("  ui_critical_extras:");
  ...
  it("[P2] keeps visual ownership and generic full UI sharding explicit", async () => {
    ...
    expect(benchmarkWorkflow).toContain("Preserve project-workspace-collab domain artifact");
